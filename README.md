...
## ¿Qué es un commit?
Un commit es un guardado de los cambios del proyecto en tu computadora. Representa una versión específica del código con una descripción que indica qué se modificó. Permite llevar el control de la evolución del proyecto.

### ¿Cómo se hace un commit?
Primero se agregan los archivos al área de preparación y luego se guarda el commit:
```bash
git add .
...
git rebase -i  hace un rebase interactivo, que te permite combinar, editar o eliminar los últimos commits. Se usa para limpiar el historial antes de subir los cambios. Por ejemplo, git rebase -i HEAD~3 te deja revisar y modificar los tres últimos commits.
